...
import {
  ...
  createEmptyResponse,
  createErrorResponse,
  createNFTItem, updateCollection,
  updateScamGroup, updateSingle, updateSuspiciousGroup
...
async function processNFTs(
  ...
  for (const nft of nfts) {
    try {
      ...
      const groupType = spamScore === 100 ? "scam" :
        (spamScore > 0 ? "suspicious" :
          (collectionId || null));

      // Create NFT item
      const nftItem = createNFTItem(nft, metadata, groupType, creatorId, spamScore);
      ...
      if (groupType === "scam") {
        ...
        updateCollection(collections, collectionId, nftItem, marketplaceData, solPriceInfo);
      } else if (creatorId) {
        updateSingle(singles, creatorId, nftItem, solPriceInfo);
      }
    } catch (error) {
      console.error(`Error processing NFT ${nft.mintAddress?.toString()}:`, error);
